...
def process_df(df):
    ...
    advice_list = df['advice'].to_list()
    advice = ''
    for sent in advice_list:
        advice += '//' + str(sent)
    advice = advice.replace('//nan','').replace('N/A',"")
    
    
    pros_list = df['pros'].to_list()
    pros = ''
    for sent in pros_list:
        pros += '//' + str(sent)
    pros = pros.replace('//nan','').replace('N/A','')
    
    
    cons_list = df['cons'].to_list()
    cons = ''
    for sent in cons_list:
        cons += '//' + str(sent)
    cons = cons.replace('//nan','').replace('N/A','')
    
    return avg_pros,avg_cons,avg_total,advice,pros,cons
